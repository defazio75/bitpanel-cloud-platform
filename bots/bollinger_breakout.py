...
def load_strategy_allocation(coin, strategy_name, mode):
    path = f"data/json_{mode}/current/{coin}/strategy_state.json"
...
def run(price_data, coin="BTC", mode=None):
    ...
    state = load_coin_state(coin, mode).get('BOLLINGER', {})
    allocation_pct = load_strategy_allocation(coin, "Bollinger Bot", mode)
    ...
    save_bot_state(coin, 'BOLLINGER', state, mode)
...
def update_profit_json(coin, mode, coin_amount, profit_usd):
    ...
    bot_stats = data.get("bots", {}).get("bollinger_breakout", {})
    ...
    data["bots"]["bollinger_breakout"] = bot_stats
